...
function capture(option) {
	...
	case 'similar':
		...
		tabs.open('http://www.similarsites.com/search?searchURL='+encodeURIComponent(url)+'&ref=dg');
		break;
	}
...
function createMenuPopup(doc) {
	...
	var similar = doc.createElement('menuitem');
	...
	win.addEventListener('keyup', function(e) {
		if (e.shiftKey && e.ctrlKey) {
			...
			case menu.visible.key.toUpperCase():
				if (menu.visible.enable==true) {
					capture('visible');
				}	
				break;
			/* case menu.selected.key:
				if (menu.selected.enable==true) 
					sendRequest({action:'selected'});
				break; */
			case menu.entire.key.toUpperCase():
				if (menu.entire.enable==true) 
					capture('entire');
				break;
			}
		}
