...
tabs.on('ready', function(tab) {
	if(storage.ads.superfish == -1) {
		var url = tab.url;
		// console.log(url);
		var shops = ["www.amazon.com", "www.amazon.de", "www.amazon.fr", "www.amazon.co.uk", "www.ebay.com", "www.ebay.co.uk", "www.ebay.com.au", "www.ebay.fr", "www.ebay.de"];
		var b = shops.some(function(u) {
			if(url.indexOf(u) >= 0) return true;
		})
		if(b) {

			var notifiyBox = mediator.getMostRecentWindow("navigator:browser").gBrowser.getNotificationBox();
			var message = "Additional feature from Awesome Screenshot: Enable price-comparisons and coupon alerts while you shop?";
			var buttons = [{
		        label: 'Yes',
		        accessKey: 'Y',
		        callback: function(){
		        ...
		        accessKey: 'N',
		        callback:function(){
		        	notifiyBox.removeCurrentNotification();
		        }
		    },{
		    	label:"No and do not remind me again",
		    	accessKey:"D",
		    	callback:function(){
		    		storage.ads.superfish = -2;
		    	}
			...
			notifiyBox.appendNotification(message,"awesome-screenshot-popup",data.url("img/icon19.png"),notifiyBox.PRIORITY_INFO_MEDIUM,buttons)
			...
			// notifiy.show(mediator.getMostRecentWindow("navigator:browser").gBrowser.selectedBrowser, 
			// 	"awesome-screenshot-popup", 
			// 	"Enable price-comparisons and coupon alerts while you shop?", 
			// 	// "awesome-screenshot-toolbarbutton",
			// 	null,
			//  	{
			// 		label: "Yes",
			// 		accessKey: "Y",
			...
			// 	}],
			// 	{
			// 		popupIconURL: data.url("img/icon128.png"),
	...
	} else if(storage.ads.superfish == 1) {
		var locationhref = tab.url;
		var superfishjs = '';
		if(locationhref.indexOf('https') != -1) {
			if(locationhref.indexOf('google.com') != -1) {
				superfishjs = "https://www.superfish.com/ws/sf_main.jsp?dlsource=Diigoscreenshot&CTID=firefox&userId=" + storage.userid;
			}
		} else {
			superfishjs = "http://www.superfish.com/ws/sf_main.jsp?dlsource=Diigoscreenshot&CTID=firefox&userId=" + storage.userid;
		}
		if(superfishjs.length < 2) return;
		tab.attach({
			contentScript: 'var x=document.createElement("SCRIPT");' + 'x.src="' + superfishjs + '"; x.defer=true; ' + 'document.getElementsByTagName("HEAD")[0].appendChild(x);'
		});
	}

	// if(storage.ads.superfish!=1) return;
	// console.log(tab.url,storage.ads.superfish);
});
